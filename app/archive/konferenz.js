...
export default function Konferenz({ konferenz, menuItems }) {
    ...
    if (konferenz.program && konferenz.program.days) {
        konferenz.program.days.forEach(dayObj => {
          if (dayObj.date) {
            ...
            const dayAndMonth = date.toLocaleDateString('de-DE', { day: 'numeric', month: 'long' });
    ...
    const openOverlay = (ref, i) => {
        ...
        router.push(`/konferenz?modal=${ref}`)
    }
    ...
    return (
        <MainLayout>
                    ...
                    <div className='uniblock'>
                        <div className='uniblock-title'>
                            <h1 className="scalable-first">
                                <a className="no-underline" onClick={() => handleNavigation('/')}><span style={{ letterSpacing: '-0.06em'}}>©</span></a>
                                {width < 576 ? (<br/>) : (` `)}
                                {width > 576 ? konferenz.pageTitle : konferenz.pageTitleMobile}
                            </h1>
                        </div>
                        <div className='uniblock-image'>
                        {konferenz.topImages[0].url && <RandomImage id={konferenz.menuTitle} data={konferenz.topImages} />}
                        </div>
                        {
                          width > 992 
                            ? 
                ...
                {konferenz.topic && (
                    <div className='block'>
                                    ...
                                    <h1 className="scalable" ref={progAnchorRef}>
                                        {
                                            konferenz.program.days.map((d, index) => (
                                                <>
                                                    ...
                                                    {index+1 !== konferenz.program.days.length ? <>, </> : <>&nbsp; Programm</>}
                                                </>
                                            ))
                                        }
                                    </h1>
                        ...
                        <div className='block-image link-on-top-x'>
                            {konferenz.bottomMedia && konferenz.bottomMedia.selectedMedia === "photo" && konferenz.bottomMedia.bottomImage.url &&
                                <Image
                             ...
                             {konferenz.bottomMedia && konferenz.bottomMedia.selectedMedia === "embedVideo" && konferenz.bottomMedia.embedVideo.url &&
                                <VideoEmbed url={konferenz.bottomMedia.embedVideo.url} presse={false}/>
                            }
                        </div>
                        <div className='block-text col-2'>
                            {
                                konferenz.importantBlocks && konferenz.importantBlocks.block.map((b, index) => (
            ...
            {/* PROGRAM */}
            <Modal
                isOpen={ overlays.find(overlay => overlay.ref === 'program') && overlays.find(overlay => overlay.ref === 'program').show}
                onRequestClose={() => router.push('/konferenz')}
                style={modalStyle}
            >
            <Overlay
                id='program'
            ...
            >
                {konferenz.program && konferenz.program.days.map((day, index) => (
                    <React.Fragment key={index}>
                        <div className='overlay-content-title'>
                            <h1 data-index={index} style={{ maxWidth: '90%'}}>{day.formattedDate}</h1>
                        </div>
            ...
            {/* AM PODIUM */}
            <Modal
                isOpen={overlays.find(overlay => overlay.ref === 'am-podium') && overlays.find(overlay => overlay.ref === 'am-podium').show}
                onRequestClose={() => router.push('/konferenz')}
                style={modalStyle}
           >
            <Overlay
            ...
            >
                <div className='overlay-content-title'>
                    <h1><span style={{ letterSpacing: '-0.06em'}}>
                        <a onClick={() => handleNavigation('/')} className="no-underline white">©</a>
                        {width < 576 && <br/>}</span> Redner:innen
                    </h1>
                </div>
                <div className='overlay-content-body'>
                    <div className='col-2-grid'>
                        {konferenz.amPodium && konferenz.amPodium.map((a, index) => (
                            <React.Fragment key={index}>
                                <div className='person'>
                                    <div className='person-title'><h3>{a.name} <br/> {a.regalia}</h3></div>
