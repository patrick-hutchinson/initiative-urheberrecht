...
export default function Poster({ data }) {
    ...
    useEffect(() => {
      ...
      const handleLoad = () => {
          gsap.to(document.body, { opacity: 1, duration: 1.5 });
      }
    ...
    useLayoutEffect(() => {
      ...
      let ctx = gsap.context(() => {
        gsap.to(carRef.current, { opacity: 1, duration: 1.5 });
      }, galRef); 
    ...
    useEffect(() => {
      ...
      let maxHeight = window.innerWidth > 568 ? 15.5 : 14.5
      let spacing = window.innerWidth > 568 ? 0.0274 : 0.0274
    ...
    // FOR DEFAULT
    const repeatQ_DEFAULT = 3
    const repeatQ_KONFERENZ = 6

    const REPEAT_Q = data.type === 'default' ? repeatQ_DEFAULT : repeatQ_KONFERENZ

    const list_DEFAULT = []
    for (let i = 0; i < REPEAT_Q; i++) {
      list_DEFAULT.push(
    ...
    for (let i = 0; i < REPEAT_Q; i++) {
      list_KONFERENZ.push(
        <React.Fragment key={i}>
            ...
            {(data.type === 'konferenz' && data.dates) ? <li>{data.dates}</li> : <li style={{ color: 'red' }}>Enter dates in admin</li>}
        </React.Fragment>
        )
    }

    const TEXT = data.type === 'default' ? list_DEFAULT : list_KONFERENZ
  
    return (
      <>
        <container id='poster' ref={posRef}>
            {/* <div className={`gallery ${isDayTime ? `day` : `night`}`} ref={galRef}> */}
            <div className={`gallery`} ref={galRef}>
